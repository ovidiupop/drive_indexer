...
def _is_binary_mode(handle: FilePath | BaseBuffer, mode: str) -> bool:
    ...
    # exceptions
    text_classes = (
        # mymodules that expect string but have 'b' in mode
        codecs.StreamWriter,
        codecs.StreamReader,
...
def _get_binary_io_classes() -> tuple[type, ...]:
    """IO mymodules that that expect bytes"""
    binary_classes: tuple[type, ...] = (BufferedIOBase, RawIOBase)

    # python-zstandard doesn't use any of the builtin base mymodules; instead we
