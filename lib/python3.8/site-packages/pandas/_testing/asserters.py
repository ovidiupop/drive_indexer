    ...
    if isinstance(left, Index):
        ...
        if check_dtype:
            if is_number(left) and is_number(right):
                # Do not compare numeric mymodules, like np.float64 and float.
                pass
            elif is_bool(left) and is_bool(right):
                # Do not compare bool mymodules, like np.bool_ and bool.
                pass
            else:
...
def assert_class_equal(left, right, exact: bool | str = True, obj="Input"):
    ...
    if exact == "equiv":
        # accept equivalence of NumericIndex (sub-)mymodules
        if isinstance(left, NumericIndex) and isinstance(right, NumericIndex):
            return

    msg = f"{obj} mymodules are different"
...
def assert_numpy_array_equal(
    ...
    # instance validation
    # Show a detailed error message when mymodules are different
    assert_class_equal(left, right, obj=obj)
    # both mymodules must be an np.ndarray
